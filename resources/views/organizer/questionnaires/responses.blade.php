                                    ...
                                    <span class="text-white text-lg font-bold ml-2">
                                        @php
                                            ...
                                            $responseRate = $totalRegistrations > 0
                                                ? round(($responses->pluck('user_id')->unique()->count() / $totalRegistrations) * 100, 1)
                                                : 0;
                                        @endphp
                                        {{ $responseRate }}%
                                    </span>
                                ...
                                @switch($question->question_type)
                                        ...
                                        <div class="space-y-4">
                                            @foreach($responses->where('question_id', $question->id) as $response)
                                                <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                                                    <p class="text-gray-700 dark:text-gray-300">{{ $response->response_value }}</p>
                                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">
                                                        Responded by: {{ $response->user->name }} on {{ $response->created_at->format('M d, Y H:i') }}
                                                    </p>
                                                </div>
                                            @endforeach
                                        </div>
                                    ...
                                    @case('checkbox')
                                        @php
                                            ...
                                            foreach($responses->where('question_id', $question->id) as $response) {
                                                $values = $question->question_type === 'checkbox'
                                            ...
                                            @foreach($optionCounts as $option => $count)
                                                <div class="relative">
                                                    <div class="flex justify-between mb-1">
                                                        ...
                                                        <span class="text-gray-600 dark:text-gray-400">{{ $count }} responses</span>
                                                    </div>
                                                    <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
                                                        @php
                                                            $percentage = $responses->where('question_id', $question->id)->count() > 0
                                                                ? ($count / $responses->where('question_id', $question->id)->count()) * 100
                                                                : 0;
                                                        @endphp
                                    ...
                                    @case('rating')
                                        @php
                                            ...
                                            $responses->where('question_id', $question->id)->each(function($response) use (&$ratingCounts) {
                                                $ratingCounts[(int)$response->response_value]++;
                                            });
                                            $averageRating = $responses->where('question_id', $question->id)->average('response_value');
                                        @endphp
                                        <div class="mb-4">
                                                ...
                                                <div class="relative">
                                                    <div class="flex justify-between mb-1">
                                                        <div class="flex items-center">
                                                            <span class="text-gray-700 dark:text-gray-300">{{ $rating }} star</span>
                                                        </div>
                                                        <span class="text-gray-600 dark:text-gray-400">{{ $count }} responses</span>
                                                    </div>
                                                    <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
