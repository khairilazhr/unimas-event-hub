                                ...
                                <div class="overflow-x-auto">
                                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        <thead class="bg-gray-50 dark:bg-gray-700">
                                            <tr>
                                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Event Name</th>
                                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase hidden md:table-cell">Organizer</th>
                                        ...
                                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                            @foreach($events as $event)
                                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors">
                                                    <!-- Event Name -->
                                                    <td class="px-4 py-4">
                                                        <a href="{{ route('admin.events.show', $event) }}" class="font-medium text-blue-600 dark:text-blue-400 hover:underline">
                                                            {{ $event->name }}
                                                        </a>
                                                    </td>
                                                        ...
                                                        <div class="flex justify-end gap-2">
                                                            @if($event->status === 'pending')
                                                                <form method="POST" action="{{ route('admin.events.approve', $event) }}">
                                                                    @csrf
                                                                    <button type="submit" class="px-3 py-1.5 bg-green-100 text-green-800 hover:bg-green-200 dark:bg-green-800 dark:text-green-100 dark:hover:bg-green-700 rounded-md text-sm font-medium transition-colors">
                                                                        Approve
                                                                    </button>
                                                                </form>
                                                                <form method="POST" action="{{ route('admin.events.reject', $event) }}">
                                                                    @csrf
                                                                    <button type="submit" class="px-3 py-1.5 bg-red-100 text-red-800 hover:bg-red-200 dark:bg-red-800 dark:text-red-100 dark:hover:bg-red-700 rounded-md text-sm font-medium transition-colors">
                                                                        Reject
                                                                    </button>
                                                                </form>
                                                            @endif
                                                            <a href="{{ route('admin.events.show', $event) }}" class="px-3 py-1.5 bg-blue-100 text-blue-800 hover:bg-blue-200 dark:bg-blue-800 dark:text-blue-100 dark:hover:bg-blue-700 rounded-md text-sm font-medium transition-colors">
                                                                View
                                                            </a>
                                                        </div>
                                                    </td>
